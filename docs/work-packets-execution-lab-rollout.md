...
- The review surface now keeps the passed demo state compact: visible status says the demo completed/verified, the completion plan and fill trace are collapsed by default, stale scaffold-placeholder review flags are deduplicated into a collapsed debug warnings section, and only meaningful human checks stay visible.
