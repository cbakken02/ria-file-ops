...
| AUTH-001 | Critical | Auth | OAuth access tokens can appear in the browser-visible NextAuth session contract. | Browser-visible Drive-scoped tokens can expose user Drive access and increase blast radius of XSS or client compromise. | `auth.ts`; `types/next-auth.d.ts`; session consumers | `auth-security-audit.md`; `security-remediation-roadmap.md`; `security-roadmap.md` | Public NextAuth session output and types no longer include provider access/refresh tokens or Drive token-derived fields. Login OAuth tokens are not copied into the JWT/session contract; Drive authorization uses the server-side storage OAuth flow. Keep regression tests and rotate/revoke Google grants if prior exposure is suspected. | Mitigated / monitor | Engineering | Release check |
...
| DRIVE-001 | High | Google Drive / OAuth | Legacy Drive `signIn()` path can reintroduce browser-visible Drive tokens. | It can request Drive scope through NextAuth and flow tokens into the public session contract. | `components/google-drive-connect-button.tsx`; NextAuth provider flow | `auth-security-audit.md`; `security-remediation-roadmap.md`; `security-roadmap.md` | The legacy component now redirects to `/api/storage/google/start`; Drive scope is requested only by the server-side storage OAuth route. Keep tests preventing Drive-scoped `next-auth/react` sign-in from returning. | Mitigated / monitor | Engineering | Release check |
...
3. `AUTH-001` - OAuth access tokens can appear in the browser-visible NextAuth session contract.
   ...
   Current state: public NextAuth session output and types no longer include provider access/refresh tokens or Drive token-derived fields. The legacy Drive sign-in button now uses the server-side storage OAuth route. Release check: keep AUTH-001 regression tests passing and rotate/revoke Google grants if prior exposure is suspected.
...
- `AUTH-001`: mitigated in code for the public session contract; keep token-exposure regression tests and any needed Google grant rotation review as release checks.
- `AUTH-002`, `AUTH-003`: open sign-in and unclear session/account policy.
